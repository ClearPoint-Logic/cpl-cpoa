...
| 0:32–1:12 | **Ready path** | Agent Zoo → run `safe_research_agent` → run screen | "Pick a candidate. The agents inspect it, propose a policy envelope, run the validation suite, and issue a Passport, AI BOM, and a hash-chained evidence bundle. Clean owner, read-only tools, low risk — **Ready**, score 100." |
