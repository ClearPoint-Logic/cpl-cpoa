...
| 5 | ADK / Gemini / Stitch demonstrable | ✅ ADK+Gemini · 🧑 Stitch designs | `agents/`; Stitch design files are a Stream-C deliverable (`design/stitch/`) |
...
| 8 | Web UI deployed behind basic auth | ⏳ | `infra/cloudrun/deploy.sh`; verify after deploy |
...
| 21 | Hosted demo works for 5 must-ship | ⏳ | verify after deploy |
...
| 28 | Grounded comparison in deployed UI citing a public source | ✅ data · ⏳ UI panel | `/api/grounding-comparison` + policy grounding chips; `/grounding` route + redeploy |

## Remaining before submission
- **Deploy verification** (#8, #21): run all 5 must-ship through the hosted UI; confirm basic auth.
